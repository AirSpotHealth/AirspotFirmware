 ...
 * @LastEditTime: 2023-03-17 14:14:07
 * @FilePath: \nRF5_SDK_17.1.0_ddde560\examples\dfu\secure_bootloader\led.c
 * @Description: 
 */
#include "led.h"
...
static void led_app_timer_elapsed_handler(void* args)
{
    ...
    //3通道点亮白光
    LED_CH_OFF(led_pins[led_on_index][0]); 
    LED_CH_OFF(led_pins[led_on_index][1]);
    LED_CH_OFF(led_pins[led_on_index][2]);
...
void led_init(void)
{
    ...
    app_timer_start(led_app_timer, APP_TIMER_TICKS(250), NULL);
